        ...
        array.map(movie => {
            return (
                <Link href={`/${movie.id}`} key={movie.id} className='flex flex-col gap-2 min-w-[42.5%]'>
                    <Image key={movie.id} src={`https://image.tmdb.org/t/p/original/${movie.poster_path}`} height={600} width={600} alt="movie" className="rounded-md object-cover aspect-square w-full h-full" />
